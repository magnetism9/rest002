...
const services = [
  { icon: Monitor, title: '컴퓨터 판매', desc: '개인·기업 용도에 맞는 최적의 사양을 컨설팅하고 합리적인 가격에 공급합니다.', color: 'text-[#2452c4]', bg: 'bg-[#1a3a8f]/10' },
  { icon: Code, title: '프로그램 개발', desc: '업무 자동화, ERP, 맞춤형 소프트웨어 등 비즈니스에 필요한 프로그램을 개발합니다.', color: 'text-[#166534]', bg: 'bg-[#14532d]/10' },
  { icon: Globe, title: '웹개발', desc: '반응형 웹사이트, 쇼핑몰, 관리자 시스템 등 목적에 맞는 웹서비스를 구축합니다.', color: 'text-[#991b1b]', bg: 'bg-[#7f1d1d]/10' },
]
...
const reasons = [
  { icon: Zap, title: '빠른 납기', desc: '명확한 일정 관리로 약속한 기한 내에 반드시 납품합니다.', color: 'text-[#2452c4]', bg: 'bg-[#1a3a8f]/10' },
  { icon: ShieldCheck, title: '품질 보증', desc: '납품 후에도 하자 보수 및 유지보수를 통해 지속적으로 품질을 보장합니다.', color: 'text-[#166534]', bg: 'bg-[#14532d]/10' },
  { icon: Headphones, title: '밀착 지원', desc: '담당자가 직접 소통하여 고객의 요구사항을 정확하게 파악하고 반영합니다.', color: 'text-[#991b1b]', bg: 'bg-[#7f1d1d]/10' },
  { icon: Cpu, title: 'IT 원스톱', desc: '컴퓨터 판매부터 개발, 웹까지 IT 전 분야를 한 곳에서 해결합니다.', color: 'text-[#2452c4]', bg: 'bg-[#1a3a8f]/10' },
]
...
export default function Home() {
  return (
    <div>
      {/* Hero */}
      <section className="relative min-h-screen flex items-center theme-hero overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <div className="flex flex-wrap gap-3">
                <Link to="/contact" className="px-6 py-3 text-white font-semibold rounded-lg flex items-center gap-2 btn-primary">
                  프로젝트 문의 <ChevronRight size={18} />
                </Link>
                <Link to="/services" className="px-6 py-3 btn-primary-outline font-semibold rounded-lg">
                  서비스 보기
                </Link>
              </div>
            </div>
            ...
            <div className="hidden lg:block">
              <div className="bg-black/30 border border-white/10 rounded-2xl p-6 backdrop-blur-sm font-mono text-sm">
      ...
      {/* Stats */}
      <section className="py-10" style={{ backgroundColor: 'var(--c-bg-nav)' }}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="grid grid-cols-3 gap-4 text-center">
            {[
              ...
              <div key={s.label}>
                <s.icon size={24} className="mx-auto mb-2 theme-primary-text" />
      ...
      <section className="py-20 theme-section">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold mb-3 theme-heading">사업분야</h2>
            <p className="theme-muted">IT 전 분야에 걸친 전문 서비스를 제공합니다</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {services.map((svc) => (
              <div key={svc.title} className="p-6 rounded-xl border hover:shadow-lg transition-all group cursor-pointer theme-card">
                ...
                <h3 className="font-bold text-lg mb-2 theme-heading">{svc.title}</h3>
                <p className="text-sm leading-relaxed mb-4 theme-body">{svc.desc}</p>
                <Link to="/services" className="inline-flex items-center gap-1 text-sm font-medium hover:gap-2 transition-all theme-primary-text">
      ...
      <section className="py-20 theme-section-alt">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold mb-3 theme-heading">왜 구자성 IT인가</h2>
            <p className="theme-muted">고객 만족을 최우선으로 생각합니다</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {reasons.map((r) => (
              <div key={r.title} className="rounded-xl p-5 border hover:shadow-md transition-shadow theme-card">
                <div className={`w-11 h-11 ${r.bg} rounded-xl flex items-center justify-center mb-3`}>
                  <r.icon size={22} className={r.color} />
                </div>
                <h3 className="font-bold mb-1.5 theme-heading">{r.title}</h3>
                <p className="text-sm leading-relaxed theme-body">{r.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 theme-section">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-10">
            <h2 className="text-3xl font-bold mb-3 theme-heading">기술 스택</h2>
            <p className="theme-muted">검증된 기술로 안정적인 솔루션을 구축합니다</p>
          </div>
          <div className="flex flex-wrap justify-center gap-3">
            {techStack.map((tech) => (
              <div key={tech.name} className="flex items-center gap-2 rounded-lg px-4 py-2 border transition-colors theme-card">
                <span className="font-medium text-sm theme-heading">{tech.name}</span>
      ...
      {/* CTA */}
      <section className="py-16 theme-hero">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 text-center">
          ...
          <Link to="/contact" className="inline-flex items-center gap-2 px-8 py-3 text-white font-semibold rounded-lg btn-primary">
